...
DcmFileDraft * DICOMFileFrameRegionReader::get_dicom_file(size_t index) {
  return dcm_files.at(index).get();
}

void DICOMFileFrameRegionReader::set_dicom_files(
                      std::vector<std::unique_ptr<DcmFileDraft>> dcm_files_) {
  // all files should have frames with same dimensions
  dcm_files = std::move(dcm_files_);
  if (dcm_files.size() <= 0) {
    clear_dicom_files();
    return;
  }
  const DcmFileDraft & first_dcm_file = (*dcm_files.at(0));
  if (!first_dcm_file.get_frame(0)->has_compressed_raw_bytes()) {
    clear_dicom_files();
    return;
  }
  frameWidth = first_dcm_file.get_frame_width();
  frameHeight = first_dcm_file.get_frame_height();
  imageWidth = first_dcm_file.get_image_width();
  imageHeight = first_dcm_file.get_image_height();
  // compute frames per row and column.  Not final frames may
  // not be used completely.
  frames_per_row = static_cast<int64_t>(
                                    std::ceil(static_cast<double>(imageWidth) /
                                    static_cast<double>(frameWidth)));
  frames_per_column = static_cast<int64_t>(
                                    std::ceil(static_cast<double>(imageHeight) /
...
bool DICOMFileFrameRegionReader::get_frame_bytes(int64_t index,
    ...
    const int64_t file_count = dcm_files.size();
    for (int64_t file_idx = 0; file_idx < file_count; ++file_idx) {
      const DcmFileDraft & dcm_file = (*dcm_files.at(file_idx));
  ...
  void DICOMFileFrameRegionReader::copy_region_from_frames(
      ...
      int64_t frame_offset = fy * frameWidth;  // y offset to read from.
      ...
      for (int64_t row = 0; row < copy_height; ++row) {
        ...
        for (int64_t column = mx; column < end_mx; ++column) {
            ...
            if (imageX >= imageWidth ||   // if frame memory is null or
                imageY >= imageHeight ||  // outside of image bounds
  ...
  bool DICOMFileFrameRegionReader::read_region(int64_t layer_x,
    ...
    const uint64_t frame_mem_size_bytes = frameWidth * frameHeight *
                                                       sizeof(uint32_t);
    std::unique_ptr<uint32_t[]> frame_mem = std::make_unique<uint32_t[]>(
                                                    frameWidth * frameHeight);
    // compute first and last frames to read.
    const int64_t first_frame_y = (layer_y / frameHeight);
    const int64_t first_frame_x = (layer_x / frameWidth);
    ...
    const int64_t frame_start_x_init = layer_x % frameWidth;
    int64_t frame_start_y = layer_y % frameHeight;
    int64_t frame_yc_offset = first_frame_y * frames_per_row;
    ...
    for (int64_t frame_yc = first_frame_y; frame_yc <= last_frame_y;
      ...
      const int64_t height_copied = std::min(frameHeight - frame_start_y,
                                            mem_height - my_start);
      ...
      for (int64_t frame_xc = first_frame_x; frame_xc <= last_frame_x;
        ...
        if ((frame_xc <= frames_per_row) && (frame_yc <= frames_per_column)) {
        ...
        // or remaining in memory buffer.  Which ever is smaller.
        const int64_t width_copeid = std::min(frameWidth - frame_start_x,
                                             mem_width - mx_start);
      ...
      frame_yc_offset += frames_per_row;
    }
    return true;
  }
}  // namespace wsiToDicomConverter
