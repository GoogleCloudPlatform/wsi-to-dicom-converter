...
void WsiToDcm::initOpenSlide() {
  ...
  openslideMPP_X_ = getOpenSlideDimensionMM("openslide.mpp-x");
  openslideMPP_Y_ = getOpenSlideDimensionMM("openslide.mpp-y");
    ...
    std::sort(levelProcessOrder.begin(),
     ...
     for (size_t idx = 0; idx < levelProcessOrder.size(); ++idx) {
      ...
      if (std::get<2>(levelProcessOrder[idx+1])) {
        ...
        // Memory optimization, not generating pyrmaid from
        // a pre-loaded synthetic base layer and
...
int WsiToDcm::dicomizeTiff() {
  ...
  const double levelWidthMM = getDimensionMM(
                          largestSlideLevelWidth_ - initialX_, openslideMPP_X_);
  const double levelHeightMM = getDimensionMM(
                        largestSlideLevelHeight_ - initialY_, openslideMPP_Y_);
  clearOpenSlidePtr();
